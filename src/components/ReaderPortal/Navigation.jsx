...
const Navigation = ({ activeTab, onTabChange, isSticky }) => {
  const navItems = [
    ...
    { id: 'newsletters', label: 'Newsletters', icon: FiMail },
    // { id: 'revenue', label: 'Revenue', icon: CiViewList }
  ];

  return (
    <nav
      className={`bg-white border-b border-gray-200 px-4 lg:px-32 transition-all duration-300 ${isSticky ? 'fixed top-0 left-0 right-0 z-40 shadow-md' : 'relative'
        }`}
    >
        ...
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 py-4 border-b-2 transition ${activeTab === item.id
